...
const InputForm: React.FC<InputFormProps> = ({ onSubmit }) => {
  ...
  const defaultData: FormDataType | undefined = defaultDataStr
  ...
  const [occupancyRatePercent, setOccupancyRatePercent] = useState<string>(
    defaultData?.occupancyRatePercent || '95',
  );
  ...
  useEffect(() => {
    const isFormComplete = () => {
      return (
        ...
        apr !== '' &&
        parseFloat(apr) > 0 &&
        parseFloat(apr) <= 100 && // Ensure apr is between 0 and 100
        termYears !== '' &&
        parseInt(termYears, 10) > 0 && // Ensure termYears is greater than 0
        revenueStreams.every((stream) => stream.value !== '' && stream.count !== '') &&
        occupancyRatePercent !== '' &&
        parseFloat(occupancyRatePercent) >= 0 &&
        parseFloat(occupancyRatePercent) <= 100 && // Ensure occupancyRate is between 0 and 100
        propertyTax !== '' &&
        insuranceCost !== '' &&
        managementExpensePercent !== '' &&
        parseFloat(managementExpensePercent) >= 0 &&
        parseFloat(managementExpensePercent) <= 100 && // Ensure managementCost is between 0 and 100
